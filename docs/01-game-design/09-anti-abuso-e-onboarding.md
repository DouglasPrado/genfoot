...
> **Decisão ratificada — [R-50](../99-decisoes/registro-de-decisoes.md#6-série-r--resolução-de-pendências-2026-07-11):** um clube existente fica **disponível para ser assumido** quando cumpre, cumulativamente: (a) está **sob controle da IA** (sem usuário) há pelo menos um período mínimo de estabilização — sugestão: **1 temporada** ou o intervalo entre janelas, para não expor um clube recém-abandonado à recaptura imediata (Decisão 1904); (b) **não foi preparado por conta relacionada** ao candidato (auditoria de contexto da Decisão 1905); (c) está com o **estado consistente** — sem quarentena de ação, reversão ou auditoria aberta. A tela de entrada apresenta **criar × assumir** lado a lado, cada opção com seu **cartão de risco** em linguagem qualitativa (Decisões 1879–1880): "criar" = começo mais justo, porém lento; "assumir" = herda elenco, histórico **e também dívidas/obrigações**, podendo já estar em crise. Racional: transforma as diretrizes já decididas (1904/1905) em critério operável e torna a escolha informada, sem prometer que assumir é atalho — coerente com "assumir clube forte não é abuso, mas passa por auditoria de contexto". Onde o clube novo entra já está resolvido: **Liga Inicial** das divisões por nível estrutural (ver [`../02-tecnico/03-multiplayer-e-mundos.md`](../02-tecnico/03-multiplayer-e-mundos.md)), com o apoio do **Programa de Clube Novo** (seção [2.8](#28-programa-de-clube-novo-catch-up-estrutural)).
...
> **Decisão ratificada — [R-51](../99-decisoes/registro-de-decisoes.md#6-série-r--resolução-de-pendências-2026-07-11):** o caixa inicial do **clube novo** é **fixo e idêntico para todos**, sem variar por divisão ou porte (regra `ECO-001`; ver [`./01-mundo-persistente-e-clubes.md §3.1`](./01-mundo-persistente-e-clubes.md#31-caixa-inicial-fixo-e-idêntico)). **Não** se adotam micro-variações de caixa: a identidade entre clubes vem da distribuição de atributos, não do dinheiro de largada. Valor de referência (unificado em [R-43](../99-decisoes/registro-de-decisoes.md)): **R$ 5.000.000** (a antiga referência de R$ 1.000.000 fica superada), a calibrar no [`Catálogo de Regras e Fórmulas`](../02-tecnico/05-catalogo-de-regras-e-formulas.md). Ao **assumir um clube existente**, o usuário **não** recebe caixa fixo: herda o **caixa e as dívidas/obrigações reais** daquele clube (que podem ser negativos), pois o caixa fixo é a justiça de largada do clube novo, não um bônus de entrada. Racional: mantém a justiça inicial e evita que a entrada gere vantagem ou distorção de mercado (coerente com `ECO-001` e com "todos nascem pequenos").
...
> **Decisão ratificada — [R-52](../99-decisoes/registro-de-decisoes.md#6-série-r--resolução-de-pendências-2026-07-11):** a entrada em temporada avançada **nunca reescreve** uma tabela em curso. Dois caminhos: **(a) criar clube** → o usuário não é inserido no meio de uma liga madura; entra na **próxima Liga Inicial** disponível (coorte que abre continuamente) ou numa **liga nova** (ver [2.9](#29-escolha-da-liga-na-entrada)), começando do zero com calendário completo; **(b) assumir clube** → herda **exatamente** a situação do clube da IA — pontos, jogos disputados, calendário restante, competições em curso e classificação —, sem reset e sem vantagem sobre os adversários. Racional: preserva a integridade competitiva (Decisão 1947) e a história do mundo — o recém-chegado ou começa um ciclo próprio (Liga Inicial) ou continua uma história já existente, nunca distorce uma disputa alheia em andamento. Esta decisão de entrada tardia abrange também os objetivos ([2.4](#24-objetivos-justos)) e a pré-temporada ([2.5](#25-pré-temporada-ativa)).
...
> **Decisão ratificada — [R-52](../99-decisoes/registro-de-decisoes.md#6-série-r--resolução-de-pendências-2026-07-11):** os objetivos do novo usuário são **calibrados ao contexto de entrada**, não a um padrão absoluto — função do **porte/nível institucional** do clube, da **divisão** e do **calendário restante**. Ao **criar** (Liga Inicial): objetivos simples e de desenvolvimento (evoluir estrutura, formar base, não rebaixar). Ao **assumir** no meio da temporada: meta **proporcional ao que resta** — se o clube está em crise, o objetivo é estabilizar/recuperar, não brigar por título —, evitando frustração inicial (Decisão 1961). Racional: mesma decisão de entrada tardia (R-52, [2.3](#23-entrada-em-temporada-avançada)); metas herdadas de um clube em situação ruim não podem punir quem acabou de assumir.
...
> **Decisão ratificada — [R-52](../99-decisoes/registro-de-decisoes.md#6-série-r--resolução-de-pendências-2026-07-11):** ao **criar** um clube, o usuário recebe uma **pré-temporada ativa** antes da primeira rodada — janela curta (sugestão: **~2 semanas virtuais**) com **amistosos limitados** (sujeitos aos limites anti-farm da Decisão 1909), ajuste de elenco, definição tática e treino inicial. Ao **assumir** um clube em temporada já em andamento, **não há** pré-temporada retroativa: o usuário entra direto no calendário restante (R-52, [2.3](#23-entrada-em-temporada-avançada)), com um breve **período de adaptação** — sem amistosos oficiais extras — para conhecer o elenco. Racional: a pré-temporada é ferramenta de preparação de quem começa um ciclo, não um bônus de tempo para quem assume uma disputa em curso — coerente com a não-reescrita da tabela.
...
> **Decisão ratificada — [R-53](../99-decisoes/registro-de-decisoes.md#6-série-r--resolução-de-pendências-2026-07-11):** contra a **captura oportunista de clubes fortes**: (a) **cooldown de disponibilidade** — um clube abandonado só reabre para ser assumido após o período de estabilização de R-50 (sugestão: **1 temporada**), quebrando o ciclo de abandono-e-reassunção coordenado (Decisão 1904); (b) **reserva de vaga com TTL** durante o onboarding, reusando o `ClubEntryReservation` de [R-25](../02-tecnico/10-catalogo-de-commands.md) (sugestão: **30 min**), para que dois usuários não travem o mesmo clube; (c) **auditoria de disponibilidade** antes de liberar (Decisão 1905): verifica preparação por conta relacionada, descarga de dívidas e transferências recentes suspeitas; (d) **proteção contra escolhas quebradas** — o preview do clube exibe dívidas, folha, moral e estado (cartão de risco de R-50) e **bloqueia** assumir clube em estado inconsistente (quarentena ou reversão aberta). O **"onde entrar"** já está resolvido: clube novo entra na **Liga Inicial** (divisões por nível estrutural, ver [`../02-tecnico/03-multiplayer-e-mundos.md`](../02-tecnico/03-multiplayer-e-mundos.md)) com o **Programa de Clube Novo** (seção [2.8](#28-programa-de-clube-novo-catch-up-estrutural)); a Série R fecha apenas os parâmetros anti-captura. Racional: liga o onboarding ao antiabuso já decidido, dando números operáveis sem revelar fórmula.
...
As duas pontas soltas do escopo original ficam reconciliadas. A **"proteção contra escolhas quebradas"** deixa de ser pendência: é atendida pelo cartão de risco no preview e pelo bloqueio de clubes em estado inconsistente (R-50 na [2.1](#21-criar-clube-novo-ou-assumir-existente) e R-53 na [2.6](#26-evitar-exploração-de-clubes-fortes)). E o termo **"divisão de expansão"** é, no vocabulário canônico do Grinta, a **Liga Inicial** (seção [2.8](#28-programa-de-clube-novo-catch-up-estrutural) e [`../02-tecnico/03-multiplayer-e-mundos.md`](../02-tecnico/03-multiplayer-e-mundos.md)) apoiada pelo **Programa de Clube Novo** — não existe um mecanismo separado com aquele nome. A preservação da história do mundo em si já está resolvida pelas Decisões 1957–1958 e 1961.
...
> **Decisão ratificada — [R-54](../99-decisoes/registro-de-decisoes.md#6-série-r--resolução-de-pendências-2026-07-11):** parâmetros de 1ª passada do Programa de Clube Novo — vigentes nas **3 primeiras temporadas** do clube, com **decaimento linear** até zerar: **custo de upgrade estrutural −40%** (níveis 1→3); **tempo de construção −50%**; **bônus de base +10%** na qualidade de formação (pequeno, sem "jogador apelão"); **contratos iniciais protegidos** — jovens da base não podem ser assediados na 1ª temporada e têm a cláusula de saída endurecida para conta relacionada; **premiação de Liga Inicial ×1,5** proporcional. O decaimento ao longo das 3 temporadas evita dependência do benefício. Racional: entrega o "catch-up" estrutural sem quebrar a justiça inicial (caixa fixo idêntico R-51, elenco equilibrado R-57); números a calibrar no [`Catálogo de Regras e Fórmulas`](../02-tecnico/05-catalogo-de-regras-e-formulas.md).

> **Elegibilidade anti-farming — [R-161](../99-decisoes/fechamentos-complementares-2026-07-13.md):** o benefício pertence ao **`clubId` criado** e é concedido **uma vez por clube**; um **participante** recebe benefício de criação **no máximo uma vez por mundo**, em janela móvel de **3 temporadas**. Abandono **não** reinicia nem transfere o benefício; retornar ao mesmo clube **continua** o cronômetro. Transferências entre **contas relacionadas** durante a proteção são bloqueadas/auditadas. Benefício indevido é revertido por **lançamento compensatório** no razão, nunca por edição de saldo (fecha A-09/farming).
