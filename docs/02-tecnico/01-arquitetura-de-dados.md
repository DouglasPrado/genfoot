...
> **Status:** CANÔNICO · **Fontes:** chats/arquitetura-jogo.md, chats/ux-do-jogo.md (Bloco 25) · **Revisão:** 2026-07-10
...
- [Mensageria durável (broker — ver R-78)](#mensageria-durável-broker--ver-r-78)
...
> **Decisão ratificada — R-82:** catálogo de escalas internas por tipo de valor. Proposta de 1ª passada: **atributos oficiais** e **multiplicadores determinísticos** em escala inteira **0–10000** (a exibição pública deriva por arredondamento/faixa, sem expor o valor exato); **probabilidades oficiais** em base **10000** (`10000` = 100%, `0` = 0%, sem ambiguidade de extremos); **percentuais financeiros/contratuais** em **pontos-base** (base 10000, ex.: `500` = 5%); **taxas de câmbio** com base declarada e taxa histórica preservada. Racional: uma base única (10000) para atributos, probabilidades e percentuais reduz erros de conversão e mantém a precisão interna acima da exibição. Cada valor declara sua escala no contrato; **mudar de escala exige versionamento explícito** (§26.4), sem mistura silenciosa de unidades. Os limites por atributo específico saem com a calibração do catálogo de fórmulas (`./05-catalogo-de-regras-e-formulas.md`).
...
## Mensageria durável (broker — ver R-78)
...
> **Decisão ratificada — R-78:** o broker oficial é decidido em [`./00-arquitetura-geral.md`](./00-arquitetura-geral.md) (R-78): **Redis + BullMQ** na fundação, evoluindo para **RabbitMQ** (exchanges/filas quorum descritas nesta seção) ou **NATS** (publicação pós-commit, Decisão 19.10) quando volume e roteamento justificarem. A modelagem de **Outbox/Inbox** abaixo é **independente do broker** e vale para qualquer um deles: as exchanges/routing keys desta seção são o **desenho-alvo durável**; sobre BullMQ, os mesmos eventos trafegam como jobs em filas nomeadas equivalentes.
...
> **Decisão ratificada — R-79:** estratégia de busca em duas fases. **Início:** **PostgreSQL Full Text Search + índices trigram** (Bloco 25) — sem dependência operacional nova, suficiente para o volume inicial, e a busca não é autoritativa (só localiza a entidade; a abertura carrega o estado oficial). **Evolução:** migrar para **Meilisearch** (Decisão 19.9) — ou OpenSearch — quando volume, latência, relevância ou operação justificarem, alimentado por projeções/índices a partir do PostgreSQL. Racional: adia custo operacional sem fechar a porta a um motor externo; o contrato de consulta de busca é o mesmo nas duas fases, então a troca não afeta o domínio.
