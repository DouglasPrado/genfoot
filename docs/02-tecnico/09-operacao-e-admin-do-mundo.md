...
> **Pendência:** layout, granularidade de *drill-down*, limiares de alerta por item e definição de quais indicadores viram alertas ativos (versus apenas visíveis) ainda não estão fechados.
...
> **Pendência:** periodicidade de cada verificação, faixas-alvo (limiares saudáveis) por eixo, e o mecanismo formal de "atuar sobre gerações e regras futuras" (quais parâmetros são ajustáveis, por quem e com qual governança) permanecem em aberto.
...
> **Pendência:** comunicação ao usuário após correções (o que é notificado, em que canal e com qual nível de detalhe) ainda não está definida.
...
> **Pendência:** os **níveis finais de permissão** (mapeamento definitivo entre papéis e ações), os procedimentos de atendimento e recurso e os prazos de revisão ainda estão em aberto (fonte: Seção 25.4 do escopo definitivo).
...
> **Pendência:** metodologia formal (número de mundos/temporadas simulados por rodada de teste, métricas de aceitação e critérios de *gate* antes de promover mudanças de regra) ainda não está definida.
