...
- **Componentes e dados:** por linha (`PlayerRow`): foto, nome, idade, **posição/função**, overall (faixa), **moral**, **forma/condição física**, **fadiga**, status (lesionado/suspenso/convocado/à venda/emprestado), contrato (tempo restante). Cabeçalho com **relatório de elenco** ([doc 13, §5.2](../01-game-design/13-relatorios-notificacoes-e-memoria.md)): profundidade por posição, idade, liderança, lacunas, excesso, jogadores em risco, jovens próximos do profissional.
...
- **Referências:** [`02-jogadores`](../01-game-design/02-sistema-de-jogadores.md); [`13-relatorios §5.2`](../01-game-design/13-relatorios-notificacoes-e-memoria.md).
...
- **Layout:** cabeçalho (foto, nome, idade, posição/função, nacionalidade, overall/potencial) → abas: **Resumo · Atributos · Contrato · Desenvolvimento · Memória · Médico**.
- **Componentes e dados (Resumo):** origem, nacionalidade/região, história familiar/condição social, pé dominante, altura/peso, **personalidade e traços** (com **visibilidade**: visível / detectado por scout / oculto), **estados** (moral, fadiga, confiança, pressão, motivação, forma, ansiedade, foco), **papel no elenco** (chave/titular/rotação/reserva/desenvolvimento/liderança/mentor), **valor de mercado**, valor de imagem.
- **Ações:** conversar (`M-CONVO`), renovar (`M-CONTRACT`), treinar (`M-TRAINING-INDIV`), listar/vender (`M-MARKET`), definir papel (`M-ROLES`), promessas (`M-PROMISES`).
- **Estados:** dados ocultos aparecem como "?" com dica de scouting; ao ver jogador de outro clube, campos sensíveis ficam estimados.
- **Referências:** [`02-jogadores §1–4, §13, §15`](../01-game-design/02-sistema-de-jogadores.md).
...
- **Componentes e dados:** foco coletivo por área (técnico/físico/tático/mental), carga/intensidade, treino específico (defensivo/ofensivo/bola parada), recuperação; efeito modulado pela **comissão** e **CT** (níveis). Alertas de sobrecarga (risco de lesão) e de baixa carga.
...
- **Referências:** [`02-jogadores §6`](../01-game-design/02-sistema-de-jogadores.md); [`04-estrutura §3.2, §3.3, §3.8`](../01-game-design/04-estrutura-do-clube-e-staff.md).

## `M-TRAINING-INDIV` — Plano individual de treino

- **Objetivo:** treinar um jogador específico.
- **Componentes e dados:** área/atributo alvo, treino de posição/função, mentoria (com veterano/líder), recuperação; projeção de ganho e trade-off; risco por sobrecarga.
...
- **Componentes e dados:** diagnóstico (suspeita inicial → exames → diagnóstico → gravidade → faixa de recuperação → risco de retorno); opções de **tratamento** com prazo/risco; progresso pela **reabilitação (estágios 1–7)** com restrições que diminuem; avaliação de retorno ao treino (comissão) e retorno competitivo (medicina).
- **Ações:** solicitar exame; **escolher tratamento** (command); liberar para treino/jogo (com aviso de risco de recaída); administrar minutos.
- **Estados:** decisão de retorno precoce usa `HighRiskConfirm` (risco real de recaída); *loading* de exame.
- **Referências:** [`02-jogadores §16`](../01-game-design/02-sistema-de-jogadores.md); [`15-fluxos §12`](../01-game-design/15-fluxos-completos.md).
