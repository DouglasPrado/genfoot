...
- **R-201 — Ser dispensado abala o jogador: nota −4 e ânimo cai (C6).** `market:release-player` além de tirar do elenco e encerrar o contrato, DEGRADA o jogador: os 39 atributos caem 4 (e como a nota é derivada deles, R-09, a nota cai 4 na gravação) e o estado dinâmico despenca — moral −20, confiança −15, felicidade −25, ritmo −10. Modela o baque de ser dispensado. O `ReleaseUnitOfWork` ganhou o `players`. Provado: nota 57→53, moral 50→30.
- **R-200 — Vender é LISTAR; dispensar cria agente livre (valor −30%) (C6).** "Vender" = `market:list-player`: cria uma `TransferListing` (LISTED, preço = valor R-41) e o jogador FICA no elenco até alguém comprar — aparece no mercado (inclusive no do próprio clube) com o preço pedido. "Dispensar" = `market:release-player`: o jogador sai na hora e vira AGENTE LIVRE (sem clube), aparecendo no mercado como "Sem clube" com valor **−30%** (deságio de quem está sem vínculo). O read model do mercado une três grupos: scout (outros clubes), listados (sempre visíveis) e agentes livres (sem elenco). Falta: contratar agente livre de graça (transferência livre sem vendedor).
- **R-199 — Vender ao mercado credita o valor pelo faucet do sistema; dispensar é de graça (C6/C9).** Sem comprador humano, `market:sell-player` tira o jogador do elenco, encerra o contrato e CREDITA o valor estimado (R-41) no caixa — o dinheiro entra pelo mesmo faucet da dotação (`SYS:INITIAL_ENDOWMENT`): o "mercado" (clubes de IA, coletivamente) paga, como um prêmio (money entra na economia, INV Σdébito=Σcrédito preservado). `market:release-player` (dispensar) faz o mesmo SEM dinheiro. Ambos atômicos (elenco+contrato[+razão] num commit). Consequência aceita: é calibração de 1ª passada até o mercado de IA com compradores reais existir; o valor de venda = valor de mercado cheio (sem negociação). Provado: caixa +valor, razão balanceado.
- **R-198 — A base (youth) nasce na gênese: uma YOUTH_ACADEMY por clube (C8).** Jovem é jogador, base é elenco — então reusa os MESMOS repos de Player/Squad da gênese (sem porta nova). `deriveClubYouth` gera 12 jovens/clube (16–19 anos, `youthProspect`, atributos rebaixados ⇒ nota atual baixa/potencial alto), num `Squad` category YOUTH_ACADEMY. 16×12=192 jovens, 16 elencos de base (total de elencos: 16 profissional + 16 base = 32). Idempotência da base por `findSquadById` (não `findFirstTeamSquad`, que pularia a base por já haver profissional). Falta: promoção base→profissional, desenvolvimento por treino (§4), U15/U17/U20 separados, scouting da base.
- **R-197 — A comissão técnica nasce na gênese COM contrato — ao contrário do jogador (R-189) (C8).** `StaffMember` não tem `clubId`; o único vínculo staff↔clube é o `StaffContract` salariado. A R-189 adiou o contrato do jogador porque ele tinha o ELENCO como vínculo alternativo; o staff não tem projeção nenhuma — sem contrato, não há comissão de clube. Então a gênese cria o contrato, que é OBRIGAÇÃO REGISTRADA (salário como dado, nenhum `JournalEntry`, oferta monetária intacta — a folha é de C9, que lê estes contratos). Vertical A: 7 cargos/clube determinísticos (Person + StaffMember + StaffContract), 16×7=112, idempotente por staffId; read model `staffForClub`. Fiação (query/tela) deferida. Falta: níveis de estrutura (§1–3), multiplicador de treino (§4), ScoutReport, mercado de staff, base (youth).
- **R-196 — A caixa de entrada é do CLUBE; a transferência a preenche no mesmo commit (C12).** A home já lê o resumo do inbox (`openNotificationCount`, `timelineCount`) mas a query `inbox` não voltou do R-175 — badge mudo. `SignPlayer` (R-192) acrescenta uma `Notification` (TRANSFER_OFFER) DENTRO da transação — o `TransferUnitOfWork` agora tem 5 repos. **Club-scoped** (`clubId`, `userId=null`): a notificação é PESSOAL do clube (vs a `Narrative` R-195 que é PÚBLICA), e resolve "de quem é" sem depender do ator. Read model `summaryForClubs`. **Fiação (query `inbox` + tela) deferida** por colidir com refatoração paralela — entregue o backend isolado (adapters + teste Postgres + prova HTTP). Falta: threads MF-0B, mark-as-read, outros geradores.
- **R-195 — A imprensa narra o fato, no mesmo commit do fato (C11).** O doc 11 §10: imprensa relata fatos REAIS, nunca inventa. O `DomainEventLog` só tem onboarding (transfers/rodadas não escrevem nele), então a fonte fiel é o ato que cria o fato. `SignPlayer` (R-192) acrescenta uma `Narrative` factual DENTRO da transação da transferência — o `TransferUnitOfWork` ganhou um 4º repo (`narratives`). Manchete determinística por fato (não duplica), intensidade 1–5 pela taxa. Query `narrative` (recentes primeiro). Falta: pautas de IA (§10), acúmulo de reputação (§11), 8 posturas (R-71), manchetes de resultado/torcida.
- **R-194 — A torcida nasce na gênese: headcount determinístico; os 8 segmentos vêm depois.** Os clubes nasciam com `fanBaseSize=0` (torcida degradada na tela do Clube). C10 vertical A materializa o TAMANHO da torcida — determinístico por `(seed, clubIndex)`, enviesado para baixo (f², "todos nascem pequenos"), faixa [800, 45.000] — e deixa `boardPatience=50`/`pressureLevel=0` NEUTROS (mundo sem histórico). Fronteira: a torcida mora em colunas do `Club` que C3 não escreve; C10 é dono, escrita única na transação da gênese, idempotente por clube. Falta (C10 pleno): os 8 `FanSegment` (R-68), satisfação/histerese (R-69), expectativa, pressão por vocalidade, rivalidades (R-70), `BoardPromise`.
- **R-193 — O elenco nasce com 23, mas o teto de registro é 30.** `SQUAD_SIZE=23` era preenchimento da gênese E teto rígido do `assign` ao mesmo tempo — todo clube nascia cheio e a primeira contratação (R-192) batia em `SQUAD_CAPACITY_EXCEEDED`: o mercado nascia travado. R-57 diz "elenco *inicial* 23", não máximo. Separa-se: `SQUAD_SIZE=23` (gênese) e `MAX_SQUAD_SIZE=250` (teto de registro, mede o `Squad`). Folga de 7 vagas permite contratar sem antes vender.
- **R-192 — A transferência é um efeito atômico sobre C6+C9+C3, e paga a dívida da R-189.** Um só commit: dinheiro anda (JournalEntry TRANSFER, comprador→vendedor), contrato nasce (PlayerContract autoritativo, Q5), elenco segue (SquadMembership move). Taxa em [40%,250%] do valor (R-26). Meio efeito é corrupção.
...
### R-202..R-207 — Competições autoradas no admin, imutáveis ao iniciar (C7) · RATIFICADAS em 2026-07-18

A liga era cravada na gênese (16 clubes, 30 rodadas, hardcoded). A spec §2 já modelava campeonatos como entidades independentes com regras/qualificação/prêmios (R-59/R-61/R-62/R-63), mas nada configurável. Decisão de produto: competição vira **autorada no admin, configurada antes de iniciar e congelada ao iniciar** — a materialização administrável do que a spec desenhou, com a trava da R-52 estendida a toda a config.

- **R-202 — Competição é agregado autorado, com ciclo de vida e config imutável ao iniciar.** `Competition` deixa de ser efeito da gênese: RASCUNHO (config editável) → `lock` (valida config, materializa sorteio+datas) → AGENDADA → EM_ANDAMENTO → ENCERRADA. De AGENDADA em diante, todo command de config falha (`COMPETITION_LOCKED`, R-52). `version` por linha (R-175).
- **R-203 — O mundo nasce SEM competição; a gênese cria só o pool de clubes (`clubCount` parametrizável, múltiplo de 20, padrão 20).** A liga hardcoded morre; a gênese materializa só clubes/jogadores/elenco/base/comissão/torcida/economia. O admin agrupa os clubes em divisões ao configurar. Mobile ganha estado vazio.
- **R-204 — Liga tem divisões de 20; acesso/rebaixamento 4 sobem / 4 descem, em cascata.** Divisão = 20 clubes (turno e returno ⇒ 38 rodadas/380 jogos). Fim de temporada: 4 últimos descem, 4 primeiros da de baixo sobem. Topo não é promovido; fundo não é rebaixado. Slots calibráveis (`promotionSlots`/`relegationSlots`).
- **R-205 — Premiação entra no razão (C9) em camadas, configurada antes de iniciar.** Faucet `SYS_PRIZE_FAUCET` na homologação: posição final + cota de participação + bônus por vitória/rodada (clubes) + prêmios individuais (creditados ao clube do jogador). Efeitos de reputação/mercado seguem R-59/R-61.
- **R-206 — Copa em dois formatos: mata-mata direto e grupos + mata-mata.** `COPA_MATA_MATA` (chaveamento semeado por reputação) e `COPA_GRUPOS_MATA_MATA` (potes → grupos → mata-mata). Sorteio e datas materializados no `lock`, determinísticos por `(seed, competitionId)`.
- **R-207 — Qualificação entre competições é declarada na config e imutável.** `QualificationRule` (`sourceCompetitionId`, `criteria`, `slots`) é parte da config imutável (R-202): decidida antes, aplicada na homologação (top-N → vagas). Não muda depois que qualquer competição envolvida inicia.
...
### R-217 — Declínio físico e aposentadoria por curva de idade, na virada · RATIFICADA em 2026-07-19

O §5 e o §17 especificavam declínio por idade e aposentadoria contextual; o código não fazia nada disso (curva de idade só reduzia ganho; `RETIRED`/`PlayerRetiredEvent` eram tipos sem produtor). Decisão do dono: núcleo por **curva de idade + probabilidade**, com os fatores contextuais do §17 adiados.

- **R-217 — Declínio e aposentadoria aplicam na virada (passo 7, INV-29/R-113), deterministas.** Atributos físicos decaem por curva de idade (sem perda ≤29, gradual 30-33, queda 34+) via `applyAttributeChange` (delta negativo, clamp ±6, com piso). Aposentadoria por probabilidade crescente com a idade; o sorteio é **determinístico** (`SeededRandom` por `(worldSeed, playerId, seasonId)`, R-182), marca `RETIRED` e emite `PlayerRetiredEvent`. Os números das curvas são calibração do construtor (candidatos a VAL-001). Regeneração/reposição (R-114) NÃO entra — sem ela o elenco encolhe a cada virada (trava registrada).
...
### R-220 — Entrosamento, treino de sessão e fundação tática · RATIFICADA em 2026-07-19

O dono pediu treino em duas formas (coletivo→entrosamento, individual→atributos) e um entrosamento que buffa o time na partida. A investigação revelou que o entrosamento já é RATIFICADO nos docs (R-07 grafo, R-15 ±6, R-33, coluna tacticalCohesion, telas M-LINEUP/M-TEAMBALANCE) mas sem código, e que a proposta diverge da R-07. Três forks foram batidos pelo dono:

- **R-220.1 — Fonte:** treino coletivo (por formação) **+** minutos jogados juntos. Emenda/estende a R-07 (as duas fontes somam). Treino individual não afeta entrosamento.
- **R-220.2 — Treino de sessão** substitui a execução passiva (R-212..R-216): duração em tempo real, jogador indisponível, ganho parcial ao interromper, estamina no mesmo dia. Toca R-113/INV-29 (tensão aberta, resolvida na Fase 2).
- **R-220.3 — Formação + escalação primeiro:** fundação tática é pré-requisito (a partida hoje é média do time, sem formação/escalação no domínio).

Programa em 3 fases: (1) formação/escalação; (2) treino de sessão; (3) entrosamento. Emenda R-07; reaproveita R-15/R-33; zera na transferência. A definição completa vive em [`entrosamento-e-treino-de-sessao-2026-07-19.md`](entrosamento-e-treino-de-sessao-2026-07-19.md).

### R-221 — O atributo do jogador é vivo: sobe e desce, na hora · RATIFICADA (direção) em 2026-07-19

Resolve a tensão aberta na R-220: o dono escolheu progresso **dinâmico e instantâneo**, e foi além — o atributo é **vivo**, movendo-se ± em resposta a treino, partidas e decisões, aplicado na hora. Emenda R-113/INV-29 (o desenvolvimento deixa de ser cristalizado na virada). Consequências aceitas: currentAbility/força/valor/faixa de olheiro contínuos na temporada; a virada não cristaliza mais crescimento (envelhecimento/captação seguem lá); o genfoot vira jogo de progressão de loop curto neste eixo. Teto por potencial (R-216) para cima, piso para baixo; anti-grind por tempo real de sessão + indisponibilidade. **Resolvido:** a queda por partida/decisão DECAI (forma) sobre um núcleo permanente — habilidadeEfetiva = núcleo (treino, permanente, teto por potencial) + forma (partida/decisão, ± transiente que sara). A definição completa vive em [`desenvolvimento-dinamico-2026-07-19.md`](desenvolvimento-dinamico-2026-07-19.md).
