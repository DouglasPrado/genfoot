...
- **Âncoras de link:** os links internos usam âncoras **sem diacríticos** (ex.: `#4-calendario-da-temporada` para "Calendário da temporada"). A doc assume um renderizador que **remove acentos** ao gerar âncoras (Docusaurus, VitePress, Astro Starlight); no preview nativo do GitHub as âncoras com acento podem não resolver. Os links são internamente consistentes entre si.
