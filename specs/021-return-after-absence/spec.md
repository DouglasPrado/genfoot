# Feature Specification: Retorno após ausência longa
...
### User Story 1 — Completar retorno após ausência longa (Priority: P1)

Como gestor, quero reintegrar quem ficou ausente com catch-up idempotente, explicação do que mudou e prioridades acionáveis, sem punição artificial, para que a jornada seja íntegra e compreensível.

**Why this priority**: este é o caminho principal de valor de GP-002 e sua conclusão prova a convergência dos contextos participantes.

**Independent Test**: o mesmo período de ausência processado novamente não duplica efeitos e produz resumo ordenado com todas as decisões automáticas e urgências.

**Acceptance Scenarios**:

1. **Given** os pré-requisitos autoritativos válidos, **When** o gestor percorre detectar ausência → concluir catch-up → consolidar mudanças do mundo/clube → separar decisões automáticas e prazos → priorizar ações → retomar controle, **Then** cada owner aplica somente sua mudança e a jornada apresenta um resultado único.
...
- retorno enquanto catch-up está em curso; múltiplos logins; lacuna de sequência; nenhuma mudança relevante; decisão automática falhou e foi retomada.
...
- Orquestração observável: detectar ausência → concluir catch-up → consolidar mudanças do mundo/clube → separar decisões automáticas e prazos → priorizar ações → retomar controle.
...
- Validação E2E da colaboração entre BC-002, BC-011, X-001 e X-003.

### Excluded

- Implementar integralmente os bounded contexts dependentes.
- Criar aggregate, tabela ou repositório próprio para GP-002.
...
**Dependencies**: BC-002, BC-011, X-001 e X-003. A jornada só inicia quando seus commands, queries, eventos e erros necessários estiverem congelados.

**Writing boundaries**: C2 possui relógio e catch-up; C11 possui resumo/notificações; X-001 emite decisões normais; X-003 apenas apresenta. GP-002 é coverage/evidência, nunca owner autoritativo.

## Requirements

- **FR-001**: A jornada MUST executar detectar ausência → concluir catch-up → consolidar mudanças do mundo/clube → separar decisões automáticas e prazos → priorizar ações → retomar controle com estado e próxima ação observáveis.
...
- **FR-010**: O fluxo MUST respeitar R-94 e INV-27…INV-31.

### Conceptual entities

AbsenceWindowView, ReturnSummary, ReturnPriority, AutomatedDecisionReference são read models/referências de jornada. Nenhum é aggregate autoritativo; veja [data-model.md](data-model.md).

## Canonical Sources & Traceability

| Requirements           | Source                                                                                                                                                                                             | Decisions/criteria   |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| FR-001, FR-005, FR-009 | `docs/01-game-design/15-fluxos-completos.md`, “2. Retorno após ausência longa”; `docs/01-game-design/07-inteligencia-artificial.md`; `docs/01-game-design/13-relatorios-notificacoes-e-memoria.md` | R-94                 |
...
| FR-010                 | `docs/01-game-design/15-fluxos-completos.md`, “2. Retorno após ausência longa”; `docs/01-game-design/07-inteligencia-artificial.md`; `docs/01-game-design/13-relatorios-notificacoes-e-memoria.md` | R-94 e INV-27…INV-31 |

**Aliases normalized**: nomes de telas e etapas são projeções; aggregates e owners usam C1…C12/X-001…X-003 do blueprint.

**Known gap**: scheduler persistente e catch-up temporal idempotente existem; resumo, IA explicável e cliente ainda faltam. A lacuna encerra somente com quickstart E2E e evidências bloqueantes verdes.
...
- As dependências BC-002, BC-011, X-001 e X-003 entregam contratos versionados antes da integração.
- X-003 permanece não autoritativo e X-001 usa os mesmos commands humanos.
- A baseline parcial é limitada a: scheduler persistente e catch-up temporal idempotente existem; resumo, IA explicável e cliente ainda faltam.
