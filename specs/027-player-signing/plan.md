# Implementation Plan: Contratação de jogador
...
Entregar contratar um jogador desde a necessidade e scouting até liquidação, contrato, inscrição e integração, com compensação segura. A integração usa contracts oficiais e projeção de jornada; cada escrita permanece com seu bounded context.

## Technical Context

**Language/Runtime**: TypeScript 5.7, Node.js 22, ESM.  
**Dependencies**: casos de uso e contratos de BC-004, BC-006, BC-007, BC-009, X-002 e X-003; Vitest; eventing/saga quando aplicável.  
...
**Scale**: uma instância correlaciona BC-004, BC-006, BC-007, BC-009, X-002 e X-003; múltiplas instâncias/mundos podem avançar concorrentemente.  
**Current vs target**: nenhuma fatia executável da contratação está entregue; alvo é uma contratação completa exatamente uma vez; falha em qualquer step compensa reserva/inscrição aplicável sem duplicar contrato ou ledger.
...
**Structure decision**: não criar módulo de domínio `player-signing`; compor portas públicas dos owners e uma projeção E2E.
...
- [contracts/README.md](contracts/README.md): `StartScoutMission`, `MakeTransferOffer`, `MakeCounterOffer`, `AcceptOffer`, `SignTransfer` (C6), com steps dirigidos a C4/C7/C9; `GetScoutReport`, `CompareTransferTargets`, `GetTransferCase`, `GetTransferSagaStatus`; `TransferAgreementReached`, `FinancialReservationCreated`, `MedicalExamCompleted`, `TransferSigned`, `PlayerRegistered`.
...
1. Congelar envelopes e erros dos owners em BC-004, BC-006, BC-007, BC-009, X-002 e X-003.
