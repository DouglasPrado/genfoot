# Feature Specification: Contratação de jogador
...
### User Story 1 — Completar contratação de jogador (Priority: P1)

Como gestor, quero contratar um jogador desde a necessidade e scouting até liquidação, contrato, inscrição e integração, com compensação segura, para que a jornada seja íntegra e compreensível.

**Why this priority**: este é o caminho principal de valor de GP-008 e sua conclusão prova a convergência dos contextos participantes.

**Independent Test**: uma contratação completa exatamente uma vez; falha em qualquer step compensa reserva/inscrição aplicável sem duplicar contrato ou ledger.

**Acceptance Scenarios**:

1. **Given** os pré-requisitos autoritativos válidos, **When** o gestor percorre identificar necessidade → observar/comparar → consultar disponibilidade → negociar oferta → reservar recursos → negociar contrato → realizar exame → validar integridade → assinar/registrar/liquidar → inscrever → integrar, **Then** cada owner aplica somente sua mudança e a jornada apresenta um resultado único.
...
- contrapropostas concorrentes; reserva expira; exame reprova; janela fecha entre steps; inscrição falha após contrato; retry após liquidação.
...
- Orquestração observável: identificar necessidade → observar/comparar → consultar disponibilidade → negociar oferta → reservar recursos → negociar contrato → realizar exame → validar integridade → assinar/registrar/liquidar → inscrever → integrar.
...
- Validação E2E da colaboração entre BC-004, BC-006, BC-007, BC-009, X-002 e X-003.

### Excluded

- Implementar integralmente os bounded contexts dependentes.
- Criar aggregate, tabela ou repositório próprio para GP-008.
...
**Dependencies**: BC-004, BC-006, BC-007, BC-009, X-002 e X-003. A jornada só inicia quando seus commands, queries, eventos e erros necessários estiverem congelados.

**Writing boundaries**: C6 possui scouting/negociação/contrato; C4 exame; C7 inscrição; C9 reserva/liquidação; X-002 orquestra SAGA-01; X-003 apresenta. GP-008 é coverage/evidência, nunca owner autoritativo.

## Requirements

- **FR-001**: A jornada MUST executar identificar necessidade → observar/comparar → consultar disponibilidade → negociar oferta → reservar recursos → negociar contrato → realizar exame → validar integridade → assinar/registrar/liquidar → inscrever → integrar com estado e próxima ação observáveis.
...
- **FR-010**: O fluxo MUST respeitar SAGA-01, R-26 e R-94.

### Conceptual entities

PlayerSigningView, TransferCaseReference, TransferSagaProgress, CandidateComparison são read models/referências de jornada. Nenhum é aggregate autoritativo; veja [data-model.md](data-model.md).

## Canonical Sources & Traceability

| Requirements           | Source                                                                                                                                                                                                                                  | Decisions/criteria   |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| FR-001, FR-005, FR-009 | `docs/01-game-design/15-fluxos-completos.md`, “8. Contratação de jogador”; `docs/01-game-design/02-sistema-de-jogadores.md`; `docs/01-game-design/03-economia.md`; `docs/02-tecnico/16-sagas-e-workflows.md`, “SAGA-01 · Transferência” | R-94                 |
...
| FR-010                 | `docs/01-game-design/15-fluxos-completos.md`, “8. Contratação de jogador”; `docs/01-game-design/02-sistema-de-jogadores.md`; `docs/01-game-design/03-economia.md`; `docs/02-tecnico/16-sagas-e-workflows.md`, “SAGA-01 · Transferência” | SAGA-01, R-26 e R-94 |

**Aliases normalized**: nomes de telas e etapas são projeções; aggregates e owners usam C1…C12/X-001…X-003 do blueprint.

**Known gap**: nenhuma fatia executável da contratação está entregue. A lacuna encerra somente com quickstart E2E e evidências bloqueantes verdes.
...
- As dependências BC-004, BC-006, BC-007, BC-009, X-002 e X-003 entregam contratos versionados antes da integração.
