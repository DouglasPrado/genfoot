...
> **Conclusão (atualizada):** **6 specs** já atingiram a barra `DELIVERED` de verdade ("reproduced scope" + adapter + gate verde): FND-001, C6 (mercado), C7, C8, C9 e X-002 (eventing). A SAGA-01 de transferência roda de ponta a ponta (C6+C9+X-002) com conservação monetária. O restante segue como **PARTIAL** (slice P1/P2) ou `PLANNED`. Próximo alvo: C1 (identidade/conta/sessão) para os golden paths de onboarding.
...
1. **Sagas cross-context:** ✅ a **máquina durável** (X-002) e a **transferência SAGA-01** (C6+C9+X-002) rodam de ponta a ponta com fencing e compensação. Falta o **onboarding SAGA-03** (C1 identidade/conta/sessão).
...
~~`C9 ledger`~~ ✅ → ~~`C7 standings/homologação`~~ ✅ → ~~`C8 partida ao vivo`~~ ✅ → ~~`X-002 saga runner`~~ ✅ → ~~`C6 transferência/empréstimo`~~ ✅ → **`C1 conta/sessão`** (onboarding/SAGA-03) → C12/C10/C11 → adapters → apps.
