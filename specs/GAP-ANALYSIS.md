...
> **Conclusão (atualizada):** **7 specs** já atingiram a barra `DELIVERED` de verdade ("reproduced scope" + adapter + gate verde): FND-001, C1 (identidade), C6 (mercado), C7, C8, C9 e X-002 (eventing). As sagas SAGA-01 (transferência) e SAGA-03 (onboarding) rodam de ponta a ponta sobre a máquina de X-002. O restante segue como **PARTIAL** (slice P1/P2) ou `PLANNED`. Próximo alvo: C12 (anti-abuso/admin) ou C10/C11 (narrativa/notificações).
...
8. **Persistência:** BC-004 + C3/C7/C8/C9/X-002/C6/C1 persistem (adapter JSON schemaVersion 12); faltam C2 scheduler-only e os `PLANNED`.
