# Data Model: Encerramento e virada de temporada

## Ownership

GP-006 não possui aggregate nem tabela autoritativa. C2 orquestra SAGA-02; C4/C6/C7/C9/C11 escrevem seus estados; C8 fornece resultados; X-002 entrega/checkpointa. Os tipos abaixo são projeções/referências descartáveis e reconstruíveis.
...
- `currentStep`: etapa observada de “resolver partidas/recursos → homologar → confirmar títulos/acessos → pagar prêmios → avaliar gestão → processar contratos → aging/aposentadoria/youth → fechar economia/história → realocar divisões → criar/publicar nova temporada → abrir pré-temporada”.
...
- `COMPLETED` requer todas as condições de uma falha injetada em cada checkpoint pode ser retomada sem duplicar prêmio, promoção, aposentadoria, geração ou nova temporada.
