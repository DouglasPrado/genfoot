...
describe("training-session-model (R-221 mobile)", () => {
  ...
  it("payload de start exige de 1 a 5 habilidades", () => {
    expect(buildStartSessionPayload({ clubId: "c", playerId: "p", attributeCodes: [] }))
      .toEqual({ error: "NO_ATTRIBUTE" });
    expect(buildStartSessionPayload({ clubId: "c", playerId: "p", attributeCodes: ["shortPassing", "finishing"] }))
      .toEqual({ clubId: "c", playerId: "p", attributeCodes: ["shortPassing", "finishing"] });
    // Dedup + limite de 5.
    expect(buildStartSessionPayload({ clubId: "c", playerId: "p", attributeCodes: ["a", "a"] }))
      .toEqual({ clubId: "c", playerId: "p", attributeCodes: ["a"] });
    expect(buildStartSessionPayload({ clubId: "c", playerId: "p", attributeCodes: ["a", "b", "c", "d", "e", "f"] }))
...
describe("buildTrainingRows — junta elenco e sessões ativas (M-TRAINING)", () => {
  ...
  const sessions = [
    { playerId: "p2", attributeCodes: ["marking"], startDate: "2027-06-08", durationDays: 7 },
  ];
  ...
  it("com sessão ativa → TREINANDO, com dias decorridos e progresso parcial", () => {
    ...
    expect(row?.session).toEqual({
      attributeCodes: ["marking"],
      elapsedDays: 3,
      durationDays: 7,
      complete: false,
      // Sem projeção do servidor nestes fixtures → lista vazia.
      projections: [],
    });
  });

  it("passa a projeção POR HABILIDADE do servidor adiante (atual → projetado)", () => {
    const rows = buildTrainingRows(
      players,
      [
        {
          playerId: "p2",
          attributeCodes: ["marking", "tackling"],
          startDate: "2027-06-08",
          durationDays: 7,
          projections: [
            { attributeCode: "marking", currentValue: 60, projectedGainPoints: 2, projectedValue: 62 },
            { attributeCode: "tackling", currentValue: 55, projectedGainPoints: 2, projectedValue: 57 },
          ],
        },
      ],
      "2027-06-11",
    ...
    const row = rows.find((r) => r.playerId === "p2");
    expect(row?.session?.attributeCodes).toEqual(["marking", "tackling"]);
    expect(row?.session?.projections).toHaveLength(2);
    expect(row?.session?.projections[0]?.projectedValue).toBe(62);
  });
  ...
  it("sessão de jogador fora do elenco é ignorada, não vira linha fantasma", () => {
    const rows = buildTrainingRows(players, [
      { playerId: "sumiu", attributeCodes: ["pace"], startDate: "2027-06-10", durationDays: 7 },
    ], "2027-06-11");
